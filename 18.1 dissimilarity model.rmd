---
title: "18.1 dissimilarity model"
author: "TG"
date: "5 4 2022"
output:
...
```{r}
breakage <- read.csv("C:\\Users\\TalG\\My Drive\\PHD\\chapter 1 strom effect\\R Scripts\\data\\breakage 2020 - 2018.csv")
...
# Add the breakage data and the coral richness



```{r}
bray_diss_resid<-left_join(bray_diss_resid,breakage_2020)
bray_diss_resid<-left_join(bray_diss_resid,stone)
```
...
ggplot(data = bray_diss_resid,aes(x=mean_c_rich,y=dissimilarity,color = Site,label=knoll_id))+geom_point()+geom_text()


ggplot(data = bray_diss_resid,aes(x=mean_c_rich,y=mean_Surface_Area,color = dissimilarity,label=knoll_id))+geom_point()+geom_text()
...
resident_diss<-lm(formula =  dissimilarity ~ Site + mean_Surface_Area +mean_coral_cover + mean_Complexity + precentage + category_depth + mean_c_rich ,data = bray_diss_resid_small, na.action = "na.fail")
...
resident_diss_interaction<-lm(formula =  dissimilarity ~ (Site + mean_Surface_Area +mean_coral_cover + mean_Complexity + precentage+ category_depth + mean_c_rich)^2 ,data = bray_diss_resid_small, na.action = "na.fail")
